...
form.addEventListener('submit', function (e) {
	...
	//console.log(typeof value); //string (black color in the Console))
	//console.log(typeof value) // number (blue color in the Console)
	//console.log(value)

	//Even though we set type="number" in form input, the value that we are getting back will be allays an string. To fix that, we can use the parseInt() function that parses a string argument and returns an integer of the specified radix (the base in mathematical numeral systems).

	const value = parseInt(amount.value);
	const random = Math.floor(Math.random() * text.length);

	/* Posible value to be entered in the input field
  empty value 
  -1 (negative number)
   > 9 (the input is mayor that the array's element's ) */
